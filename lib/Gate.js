...
class Gate {
  ...
  getCallbackId(){
    while(true){
      ...
      if(this.callbacks.has(cb_id)) continue;
      return cb_id;
    }
  }
  ...
  _deserializeCallback(cb){
    ...
    if(Array.isArray(cb)) return (...args) => {
      this._sendback([this.id].concat(cb), [...args]);
    }
  }

  _serializeCallback(cb){
    // Serialize callback here
    var cb_id = this.getCallbackId();
    this.callbacks.set(cb_id, cb);
    return [cb_id];
  }
  ...
  handleCallback(cb_id, args){
    if(!this.callbacks.has(cb_id)){
      return this.handleError("Can't find callback: " + cb_id);
    }
    let cb = this.callbacks.get(cb_id);
    this.callbacks.delete(cb_id);
    cb(...args);
  }
  ...
  _send(target, data, cb){
    ...
    if(!target.length) { // Perform local handle
      ...
      try {
        ...
        if(this.handleError){
          this.handleError(err, this._deserializeCallback(cb));
        }
        else throw err;
      }
    }
  ...
  _sendback(target, data){
    ...
    if(target.length === 1) { // Perform local handle
      if(target_id !== this.id) throw new Error(`Invalid target`);
      try {
        this.handleCallback(target[0], data);
      }
      catch(err){
        if(this.handleError){
